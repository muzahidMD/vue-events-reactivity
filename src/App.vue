  ...
  <section class="container mx-auto flex items-center flex-col">
    ...
    <form @submit.prevent action="https://google.com/search">
      <div class="container mx-auto flex space-x-5 justify-center m-5">
        <input @keyup.space="spacePressed()" name="q" type="text" class="border border-gray-500 bg-white p-5"
          placeholder="Press Space">
      </div>
      <div class="container mx-auto flex space-x-5 justify-center m-5">
        <input @keyup="pressedWhat($event)" name="text" type="text" class="border border-gray-500 bg-white p-5"
          placeholder="Type Something">
      </div>
      <button @click.prevent="buttonPressed()"
        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
        Button </button>
      <button @click.ctrl.right.prevent="ctrlRightClicked()"
        class="bg-blue-500 ml-5 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"> Ctrol + Right
        Click</button>
    </form>
