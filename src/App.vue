...
<template>
  <section class="container mx-auto flex items-center flex-col">
    <h1 class="text-center text-2xl py-10">Events & Reactivity for a Image Carousel</h1>
    <div class="">
      <!-- <div class="flex space-x-2" v-for="(image, index) in images" :key="index">
        <img class="cursor-pointer" :src="image.thumb" alt="">
      </div> -->
      <div class="flex space-x-2">
        <img @click="setImage(image)" class="cursor-pointer" v-for="(image, index) in images" :key="index"
          :src="image.thumb" alt="">
      </div>
      <div class="mt-10">
